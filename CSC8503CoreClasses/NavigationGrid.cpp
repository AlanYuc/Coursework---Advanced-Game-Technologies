...
NavigationGrid::NavigationGrid(const std::string&filename) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			infile >> type;
			n.type = type;
			n.position = Vector3((float)(x * nodeSize + 50), 0, (float)(y * nodeSize - 190));//+50,-190,跟左上方的迷宫匹配
...
//如果当前节点的f值比当前最佳节点的f值更小，就更新最佳节点和其迭代器。
//最后，将找到的最佳节点从列表中删除，并返回该节点
//从向量中擦除指针并不会删除指针所指向的对象
