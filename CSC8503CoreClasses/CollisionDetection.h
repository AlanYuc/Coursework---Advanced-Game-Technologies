...
namespace NCL {
	...
	public:
		struct ContactPoint {
			//local a b 都是基于本地坐标系，包括下面的point.localB
		...
		static Vector3 OBBCal(const Transform& worldTransform, Vector3 worldDir);
		static bool VecCal(const Vector3 delta, Vector3 plane, const Transform& worldTransformA, const Transform& worldTransformB,
			const Vector3 halfSizeA, const Vector3 halfSizeB, float& penetrationDistance,
			Vector3& normal, Vector3& pointA, Vector3& pointB);
