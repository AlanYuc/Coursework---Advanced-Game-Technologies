...
bool CollisionDetection::RayAABBIntersection(const Ray&r, const Transform& worldTransform, const AABBVolume& volume, RayCollision& collision) {
	...
	Vector3 boxSize = volume.GetHalfDimensions();//注意用的一半作为boxSize
...
bool CollisionDetection::RayOBBIntersection(const Ray&r, const Transform& worldTransform, const OBBVolume& volume, RayCollision& collision) {
	...
	//创建一个逆变换矩阵，通过方向的共轭来实现。这个逆变换用于将射线从世界空间转换到OBB的局部空间。
	Matrix3 invTransform = Matrix3(orientation.Conjugate());

	//将射线转化到跟当前worldTransform对象的局部坐标下
	Vector3 localRayPos = r.GetPosition() - position;

	//利用逆变换，创建一个在OBB局部空间内定义的新射线
	Ray tempRay(invTransform * localRayPos, invTransform * r.GetDirection());

	//Vector3()是0向量，所以这里检测在OBB局部空间中的射线和OBB是否相交，并获取碰撞信息
	bool collided = RayBoxIntersection(tempRay, Vector3(), volume.GetHalfDimensions(), collision);

	//如果发生碰撞，将碰撞点从OBB的局部空间转换回世界空间
	if (collided) {
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	//点乘的结果如果两个向量夹角超过90度，则结果是负数
	//在这里给定射线的方向，如果点乘结果为负，说明球体在射线后面
	if (sphereProj < 0.0f) {
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	...
	if (volA->type == VolumeType::Sphere && volB->type == VolumeType::AABB) {
		//注意碰撞的顺序，因为AABBSphereIntersection函数的第一个参数是AABB
		collisionInfo.a = b;
		collisionInfo.b = a;
...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	if (overlap) {
		//这里表示a物体的六个面，
		//假设碰撞发生在faces[3]这一面，那么faces[3]就正好表示该碰撞法线向量
...
bool CollisionDetection::SphereIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
	...
	if (radii > deltaLength) {
		float penetration = radii - deltaLength;
		Vector3 normal = delta.Normalised();
		Vector3 localA = normal * volumeA.GetRadius();
		Vector3 localB = -normal * volumeB.GetRadius();//这里有负号，因为分别表示两个物体的碰撞方向
...
bool CollisionDetection::AABBSphereIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	//在之前提到的，boxSize就是盒子长宽高的一半
	Vector3 boxSize = volumeA.GetHalfDimensions();

	Vector3 delta = worldTransformB.GetPosition() - worldTransformA.GetPosition();

	//clamp本应该在maths.cpp中，但老师没写到maths里面，而是单独在vector2，3，4里面
...
bool CollisionDetection::OBBIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	for (int i = 0; i < 15; i++)
	{
		if (!VecCal(relativePosition, planes[i], worldTransformA, worldTransformB, volumeA.GetHalfDimensions(), volumeB.GetHalfDimensions(), penDistance, normal, pointA, pointB))
		{
			return false;
		}
	}
...
bool CollisionDetection::VecCal(const Vector3 delta, Vector3 plane, const Transform& worldTransformA, const Transform& worldTransformB, const Vector3 halfSizeA, const Vector3 halfSizeB, float& penetrationDistance, Vector3& normal, Vector3& pointA, Vector3& pointB) {
	Vector3 AFor = worldTransformA.GetOrientation() * Vector3(0, 0, 1);
	Vector3 ARight = worldTransformA.GetOrientation() * Vector3(1, 0, 0);
	Vector3 AUp = worldTransformA.GetOrientation() * Vector3(0, 1, 0);
	...
	if (result) {
		...
		if (delta2 < penetrationDistance && plane.LengthSquared() > 0) {
			...
			normal = plane;
			pointA = OBBCal(worldTransformA, plane);
			pointB = OBBCal(worldTransformB, -plane);
		}
		return true;
	}
	return false;
...
Vector3 CollisionDetection::OBBCal(const Transform& worldTransform, Vector3 worldDir) {
	Vector3 vertex;
...
Vector3 CollisionDetection::Unproject(const Vector3& screenPos, const PerspectiveCamera& cam) {
	...
	//这里改过以后就能解决对应的camera选中出现偏移的问题
	float aspect = Window::GetWindow()->GetScreenAspect();// screenSize.x / screenSize.y;
