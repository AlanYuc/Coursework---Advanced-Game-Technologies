...
翻译：
以后我们将需要跟踪跨多帧的碰撞，所以我们将它们存储在一个集合中。

第一次它们被添加时，我们告诉对象它们正在发生碰撞。
在它们被移除的那一帧，我们告诉它们它们不再发生碰撞。

通过这个简单的机制，我们在OnCollisionBegin / OnCollisionEnd函数中建立起游戏内的交互操作
（例如，当被火箭发射器击中时减少生命值，当玩家碰到金币时增加分数等）。
*/
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		
		//对于每个碰撞信息，检查 framesLeft 是否等于 numCollisionFrames
		//如果是，表示这是第一次发生碰撞，调用对象 a 和 b 的 OnCollisionBegin 方法，通知它们开始碰撞
		if ((*i).framesLeft == numCollisionFrames) {
			i->a->OnCollisionBegin(i->b);
			i->b->OnCollisionBegin(i->a);
		}

		//const_cast 就是为了去掉 const 限定符，以允许在后续的代码中修改碰撞信息对象
		CollisionDetection::CollisionInfo& in = const_cast<CollisionDetection::CollisionInfo&>(*i);
		in.framesLeft--;

		//(*i).framesLeft = (*i).framesLeft - 1;

		//如果framesLeft小于0，表示碰撞已经结束，
		//调用对象 a 和 b 的 OnCollisionEnd 方法通知它们碰撞结束，
		//并从列表中移除这个碰撞信息
		//如果不是，说明碰撞仍在继续，保留该碰撞信息，并移动到下一个碰撞信息
		if ((*i).framesLeft < 0) {
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				//当前场景内的对象都没有设置物体的名字，连方法都没有
				//目前测试没问题，测试代码也暂时被注释掉
				//std::cout	<< " Collision between " << (*i)->GetName()
				//			<< " and " << (*j)->GetName() << std::endl;

				//将成功检测到的碰撞插入到STL::List中，
				//这将使我们能够跟踪正在发生碰撞的对象
				//以后我们可能需要在多个帧之间使用这些信息
...
这一段Physics - Collision Response的代码还是要好好看看，跟J相关的复杂公式
*/
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	//这里分别设置局部坐标的位置，且包含了碰撞法线，穿透距离的信息
	//(physA->GetInverseMass() / totalMass))这里就将物体根据自身的重量对总重量进行缩放
	//确保后续他们根据自身重量获取不同的速度
	//我们处理的是反质量，所以更重的对象具有较低的值
